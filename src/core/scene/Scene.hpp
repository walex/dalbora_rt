...
struct SCENE_LOAD_CALLBACKS {
	...
	virtual void on_geometry_attrib_loaded(RhiCommandBuffer& UNUSED_PARAM(command_buffer),
	...
	virtual void on_geometry_loaded(std::unique_ptr<Mesh> UNUSED_PARAM(mesh)) {}

	virtual void on_geometry_group_loaded(RhiCommandBuffer& UNUSED_PARAM(command_buffer), 
		size_t UNUSED_PARAM(group_id)) {}

	virtual void on_new_scene_node(RhiCommandBuffer& UNUSED_PARAM(command_buffer),
	...
	virtual void on_scene_loaded(RhiCommandBuffer& UNUSED_PARAM(command_buffer), const float3 UNUSED_PARAM(bb_min), const float3 UNUSED_PARAM(bb_max)) {}
...
public:
	...
	Scene(ResourceManager& rm): m_root_node(nullptr), m_resources_manager(rm) {}
	virtual ~Scene() = default;

	virtual void load(const std::string& scene_path, RhiCommandQueue& command_queue);
	...
	virtual void on_geometry_attrib_loaded(RhiCommandBuffer& command_buffer,
		Mesh& mesh, const std::string& name,
		const uint8_t* const data, const size_t length,
		const size_t stride, const resource_format format) override;

	virtual void on_geometry_loaded(std::unique_ptr<Mesh> mesh) override;

	virtual void on_new_scene_node(RhiCommandBuffer& command_buffer,
		SceneNode& node) override;

	virtual void on_scene_loaded(RhiCommandBuffer& command_buffer,
...
protected:
	...
	ResourceManager& m_resources_manager;
