...
void Renderer::create(RHI_DEVICE_DESC& device_desc) {
	...
	m_render_pass.create(m_device);
}

RhiSwapChain Renderer::create_swap_chain(const RhiWindow& window, const uint32_t buffer_count) {
	
	RhiSwapChain swap_chain;
	swap_chain.create(window, m_device, m_command_queue, buffer_count, true);
	return swap_chain;
}

void Renderer::draw(RhiView& surface, const RHI_VIEWPORT& view_port) {

	m_command_buffer.record([&] {

		m_render_pass.set_view_port(view_port);
		m_render_pass.set_render_target(surface);
		m_render_pass.render(m_command_buffer, [&](RhiCommandBuffer& UNUSED_PARAM(command_buffer)) {
			
			this->on_draw(surface);
		});
	});
