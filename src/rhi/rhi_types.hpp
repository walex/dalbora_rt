...
struct RHI_SWAP_CHAIN {
	RHI_TEXTURE_2D render_targets[MAX_RENDER_TARGETS];
...
struct RHI_SHADER_TABLE_ENTTRY {
	char name[MAX_SHADER_ENTRY_NAME_LENGTH];
	RHI_COMPILED_SHADER_BUFFER shader;
};
struct RHI_RT_PIPELINE : RHI_PIPELINE {
	RHI_SHADER_TABLE_ENTTRY shader_table[MAX_SHADER_TABLE_ENTRIES];
