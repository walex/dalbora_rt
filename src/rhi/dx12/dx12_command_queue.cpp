...
void dx12_command_queue_wait(RHI_COMMAND_QUEUE& command_queue) {
	...
	ID3D12Fence* i_fence = static_cast<DX_COMMAND_QUEUE&>(command_queue);
...
void dx12_command_queue_execute(RHI_COMMAND_QUEUE& command_queue, bool wait_completion, fptr_command_queue_on_execute callback) {
	...
	ID3D12CommandQueue* i_cmd_queue = command_queue;
	...
	if (list_size > 0) {
		...
		for (int i = 0; i < list_size; i++) {
			ID3D12GraphicsCommandList* i_cmd_list = *command_buffer_list[i];
