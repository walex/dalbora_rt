...
std::unique_ptr<RHI_OBJECT> dx12_rt_bvh_create(const RHI_RT_BVH_DESC& desc) {
	...
	auto ib_h = reinterpret_cast<RHI_BUFFER_RESOURCE*>(desc.geometry_buffer.get().get_index_buffer().get());
	...
	RHI_BUFFER_DESC buffer_desc(desc.device, desc.command_queue, desc.command_buffer);
