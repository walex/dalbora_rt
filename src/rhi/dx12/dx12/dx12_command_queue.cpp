...
void dx12_command_queue_execute_list(RHI_COMMAND_QUEUE_TASK& command_buffer_list, bool sync) {
	...
	if (sync)
		dx12_command_queue_wait_gpu(command_buffer_list);
}

void dx12_command_queue_wait_gpu(RHI_COMMAND_QUEUE_TASK& command_buffers) {
...
void dx12_command_queue_execute(RHI_OBJECT& queue, RHI_OBJECT& cmd_buffer, bool sync) {
	...
	RHI_COMMAND_QUEUE_TASK command_buffer_list(queue, std::move(cmd_buffers));
...
void dx12_command_queue_execute_list_synchronized(RHI_COMMAND_QUEUE_TASK& command_buffers) {
