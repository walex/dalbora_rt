...
std::unique_ptr<RHI_OBJECT> dx12_buffers_create(const RHI_BUFFER_DESC& buffer_desc) {

	auto flags = buffer_desc.is_uav == true ? D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS : D3D12_RESOURCE_FLAG_NONE;
	
	D3D12_HEAP_PROPERTIES heapProps = {};
	heapProps.Type = dx12_heap_type[buffer_desc.memory_type];
	...
	bufferDesc.Width = buffer_desc.size;
	bufferDesc.Height = 1;
	...
	HRESULT hr = dx_rhi_get_interface<ID3D12Device>(*buffer_desc.device)->CreateCommittedResource(
		&heapProps,
		D3D12_HEAP_FLAG_NONE,
		&bufferDesc,
		dx12_resource_state_type[(int)buffer_desc.initial_state],
		nullptr,
...
void dx12_buffers_upload(RHI_TRANSFER_BUFFER_DESC& desc, const bool sync) {
	...
	auto device = dx_rhi_get_interface<ID3D12Device>(*desc.device);
	auto buffer_dest_h = dx_rhi_get_interface<ID3D12Resource>(buffer_dest);
	auto buf_upd = dx_rhi_get_interface<ID3D12Resource>(*dx12_buffers_create(upd_desc).get());
	auto cmd_buffer = dx_rhi_get_interface<ID3D12GraphicsCommandList>(*desc.command_list);
	...
	cmd_buffer->CopyBufferRegion(buffer_dest_h.Get(), 0, buf_upd.Get(), 0, desc.transfer_size);
	dx12_resource_state_transition(*desc.command_list, *desc.buffer, current_state);
	if (sync)
		dx12_command_queue_execute_single_command_buffer_synchronized(*queue, *desc.command_list);
}
...
void dx12_buffers_download_synchronized(RHI_TRANSFER_BUFFER_DESC& desc) {
	...
	upd_desc.device = desc.device;
	upd_desc.size = desc.transfer_size;
	upd_desc.memory_type = buffer_memory_type_gpu_to_cpu;
	auto& buffer_src = dynamic_cast<RHI_RESOURCE&>(*desc.buffer);
	auto device = dx_rhi_get_interface<ID3D12Device>(*desc.device);
	auto buffer_src_h = dx_rhi_get_interface<ID3D12Resource>(buffer_src);
	auto buf_dwnl = dx_rhi_get_interface<ID3D12Resource>(*dx12_buffers_create(upd_desc).get());
	auto cmd_buffer = dx_rhi_get_interface<ID3D12GraphicsCommandList>(*desc.command_list);
	...
	cmd_buffer->CopyResource(buf_dwnl.Get(), buffer_src_h.Get());
	dx12_resource_state_transition(*desc.command_list, *desc.buffer, current_state);
	dx12_command_queue_execute_single_command_buffer_synchronized(*queue, *desc.command_list);
	void* mapped = nullptr;
