...
D3D12_CPU_DESCRIPTOR_HANDLE dx12_helpers_get_next_descriptor_heap_handle(const DX_DEVICE* const device, const uint32_t heap_id) {
    ...
    ASSERT_NULL(device);
    ASSERT_EXPR(heap_id < HEAP_ID_COUNT);

	DX_HEAP* heap_impl = device->heaps[heap_id].get();
    if (heap_impl->count < heap_impl->max_count)
		throw std::exception("Max descriptors reached for heap %d", heap_id);
    uint32_t* slot_id = &heap_impl->count;

    ID3D12Device* i_device = *device;
    ID3D12DescriptorHeap* i_heap = *heap_impl;
    
    D3D12_DESCRIPTOR_HEAP_DESC desc = i_heap->GetDesc();
    UINT rtvDescriptorSize =
        i_device->GetDescriptorHandleIncrementSize(desc.Type);
    D3D12_CPU_DESCRIPTOR_HANDLE h = i_heap->GetCPUDescriptorHandleForHeapStart();
    h.ptr += ((*slot_id) * rtvDescriptorSize);
    (*slot_id)++;
    return h;
}

std::unique_ptr<D3D12_CPU_DESCRIPTOR_HANDLE> dx12_helpers_get_rw_descriptor_heap_handle(ID3D12Device* i_device, ID3D12DescriptorHeap* heap, size_t slot) {
