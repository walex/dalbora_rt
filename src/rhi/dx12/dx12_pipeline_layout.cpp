...
RHI_PIPELINE_LAYOUT* dx12_pipeline_layout_create(const RHI_PIPELINE_LAYOUT_DESC* const desc) {
	...
	for (size_t i = 0; i < desc->descriptor_count; i++) {
		...
		range->BaseShaderRegister = static_cast<UINT>(descriptor.shader_register_start);
		range->NumDescriptors = static_cast<UINT>(descriptor.shader_register_max);
		range->RegisterSpace = static_cast<UINT>(descriptor.space_index);
