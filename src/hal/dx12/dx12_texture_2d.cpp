...
std::unique_ptr<HAL_OBJECT> dx12_create_texture_2d(const HAL_TEXTURE_2D_DESC& tex_desc) {
	...
	HRESULT hr = dx_hal_get_interface<ID3D12Device5>(*tex_desc.device)->CreateCommittedResource(
		&heapProps,
	...
	return std::make_unique<DX_HAL_RESOURCE>(new DX_BUFFER_HANDLE(texture));
}